...
export async function list(req, res, next) {
    try {
        ...
        const dir = (dirRaw || 'asc').toLowerCase() === 'desc' ? 'DESC' :
            'ASC';
        ...
        let where = 'WHERE 1=1';
        const params = {};
        if (q) { where += ' AND (name LIKE :q OR description LIKE :q OR sku LIKE :q)'; params.q = q; }
        if (categoryId) {
        ...
        const [items] = await pool.query(
            `SELECT SQL_CALC_FOUND_ROWS
                id,name,description,price,stock,sku,category_id,image_url,created_at,updated_at
                FROM products ${where}
                ORDER BY ${orderBy} ${dir}
                LIMIT :limit OFFSET :offset`, { ...params, limit, offset }
        );
...
export async function update(req, res, next) {
    try {
        ...
        const [[row]] = await pool.query('SELECT * FROM products WHERE id = :id', { id });
        res.json(row);
    } catch (err) {
        if (err?.code === 'ER_DUP_ENTRY') return res.status(409).json({
            message: 'SKU already exists'
